#!/bin/sh
# Runtime test for the top-layout tab strip spacing and the removed Tag API.
#
...
OUT="$TMPDIR_LOCAL/astra_topbar_strip_$$.luau"
trap 'rm -f "$OUT"' EXIT

{
	...
	echo "end)()"
	echo ""
	cat "$ROOT/scripts/topbar_strip_test.luau"
} > "$OUT"

if "$LUAU_BIN" "$OUT"; then
	echo "TOPBAR STRIP TEST PASSED"
	exit 0
else
	echo "TOPBAR STRIP TEST FAILED (see above)" >&2
	exit 1
fi
