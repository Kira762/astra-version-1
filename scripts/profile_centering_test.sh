#!/bin/sh
# Runtime test for the window + profile card centring (the pair rests centred
# as one unit, and the keep-on-screen clamp keeps both of them in view).
#
...
OUT="$TMPDIR_LOCAL/astra_profile_centering_$$.luau"
trap 'rm -f "$OUT"' EXIT

{
	...
	echo "end)()"
	echo ""
	cat "$ROOT/scripts/profile_centering_test.luau"
} > "$OUT"

if "$LUAU_BIN" "$OUT"; then
	echo "PROFILE CENTERING TEST PASSED"
	exit 0
else
	echo "PROFILE CENTERING TEST FAILED (see above)" >&2
	exit 1
fi
