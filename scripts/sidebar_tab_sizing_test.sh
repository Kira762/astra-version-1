#!/bin/sh
# Runtime test for the responsive sidebar tab-sizing behavior.
#
...
OUT="$TMPDIR_LOCAL/astra_sidebar_sizing_$$.luau"
trap 'rm -f "$OUT"' EXIT

{
	...
	echo "end)()"
	echo ""
	cat "$ROOT/scripts/sidebar_tab_sizing_test.luau"
} > "$OUT"

if "$LUAU_BIN" "$OUT"; then
	echo "SIDEBAR TAB SIZING TEST PASSED"
	exit 0
else
	echo "SIDEBAR TAB SIZING TEST FAILED (see above)" >&2
	exit 1
fi
